    ...
    <j-form-container :disabled="formDisabled">
      <a-form :form="form" slot="detail">
        <a-row>
          ...
          <a-col :span="24">
            <a-form-item label="纸厂标签重量" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input v-decorator="['lableWeigth']" placeholder="请输入纸厂标签重量"  ></a-input>
            </a-form-item>
          </a-col>
          <a-col :span="24">
            <a-form-item label="件  号" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input v-decorator="['entingGoodsId']" placeholder="请输入件  号"  ></a-input>
            </a-form-item>
          </a-col>
          <a-col :span="24">
...
<script>
  ...
  export default {
    name: 'EnteringWarehouseStatusForm',
    components: {
    },
    props: {
      //流程表单data
    ...
    data () {
      return {
        ...
        url: {
          add: "/enter/enteringWarehouseStatus/add",
          edit: "/enter/enteringWarehouseStatus/edit",
          queryById: "/enter/enteringWarehouseStatus/queryById"
        }
      }
    },
    computed: {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'brand','kind','level','weight','grand','unit','lableWeigth','entingGoodsId','bin','seller','oderId','entingWarehouseId','warehouseDate','re','signeInfo','status','link'))
        })
      },
      //渲染流程表单数据
      showFlowData(){
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'brand','kind','level','weight','grand','unit','lableWeigth','entingGoodsId','bin','seller','oderId','entingWarehouseId','warehouseDate','re','signeInfo','status','link'))
      },
    }
  }
</script>